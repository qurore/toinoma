...
export async function QaSection({
  ...
  const { data: questions } = await supabase
    .from("qa_questions")
    .select("id, title, body, pinned, question_id, created_at, updated_at, user_id")
  ...
  // Fetch answer counts and accepted-answer flags per question
  const questionIds = (questions ?? []).map((q) => q.id);

  const { data: answers } = questionIds.length > 0
    ? await supabase
        .from("qa_answers")
        .select("id, qa_question_id, is_accepted")
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        {/* Question form — visible to authenticated users */}
        {userId && (
          <>
