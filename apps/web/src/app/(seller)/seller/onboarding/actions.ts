...
export async function initStripeConnect() {
  ...
  const accountLink = await createConnectAccountLink({
    accountId,
    refreshUrl: `${process.env.NEXT_PUBLIC_APP_URL}/seller/onboarding?step=3&stripe_refresh=true`,
    returnUrl: `${process.env.NEXT_PUBLIC_APP_URL}/seller/onboarding?step=3&stripe_return=true`,
  });
